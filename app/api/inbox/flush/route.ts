...
export async function POST(req: NextRequest) {
  ...
  for (const m of due ?? []) {
    ...
    const ok = await sendEmail({
      from: await FROM(), to: m.from_addr, subject: /^re:/i.test(m.subject ?? '') ? m.subject : `Re: ${m.subject ?? 'je vraag'}`, text: m.reply,
      bcc: (await ADMIN()) || undefined, headers: m.message_id ? { 'In-Reply-To': m.message_id, References: m.message_id } : undefined,
    })
