...
| `woongezond-digest.{service,timer}` | daily, `08:00 Europe/Amsterdam` | Per-sensor report email; each contact's `report_frequency` (daily / weekly on Friday / monthly on the 1st) decides who is due (`/api/report/weekly`, see docs/rapport-weekmail-plan.md) |
...
journalctl -u woongezond-react -o cat | grep '^{' | jq 'select(.level=="error")'   # app-side errors (grep: Next' opstartregels zijn geen JSON)
```
...
de journal: `journalctl -u woongezond-react -o cat | grep '^{' | jq 'select(.scope=="client")'` (veld
